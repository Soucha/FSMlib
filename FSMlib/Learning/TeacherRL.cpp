...
vector<input_t> TeacherRL::getNextPossibleInputs() {
...
void TeacherRL::resetBlackBox() {
	_resetCounter++;
	_currState = 0;
}

output_t TeacherRL::outputQuery(input_t input) {
	_outputQueryCounter++;
	_querySymbolCounter++;
...
sequence_out_t TeacherRL::outputQuery(const sequence_in_t& inputSequence) {
	...
	for (const auto& input : inputSequence) {
		output = outputQuery(input);
		_outputQueryCounter--;
	}
	return sequence_out_t({output});// the last output
}

output_t TeacherRL::resetAndOutputQuery(input_t input) {
	resetBlackBox();
	return outputQuery(input);
}

sequence_out_t TeacherRL::resetAndOutputQuery(const sequence_in_t& inputSequence) {
	resetBlackBox();
